...
Quando a vez for de **capa com foto**, buscar em banco (Pexels + Unsplash) em vez de gerar por IA:
`node scripts/buscar-fotos.js buscar "<tema>" --orientacao portrait --n 8`. O passo 3A
da skill `/carrossel` tem o fluxo completo (previews → escolha → download + créditos).
Nesse caso a legenda leva a linha de crédito que o script imprimiu — copiada literal,
porque muda conforme o banco (o Unsplash exige o nome do fotógrafo).
