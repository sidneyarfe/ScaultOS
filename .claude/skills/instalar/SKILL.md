...
name: instalar
description: >
  Instala o ScaultOS no negócio do usuário. Entrevista sobre empresa, tom de voz,
  ...
  pedir explicitamente "rodar /instalar", "instalar o ScaultOS", "primeiro setup".
---

# /instalar — Instalação inicial do ScaultOS
...
Conferir o nome da pasta atual (`basename "$(pwd)"`). Se for `scaultos`, `ScaultOS`, `ScaultOS-main`, `scaultos-main` ou variação genérica:

> "Notei que a pasta atual ainda tem nome genérico ('<nome-atual>'). O ideal é a pasta ter o nome do seu negócio, não 'ScaultOS'. Quando terminarmos o setup, te lembro de renomear (é rápido — fechar VS Code, renomear a pasta no Finder/Explorer, abrir de novo). Bora seguir?"
...
> "Pronto. O ScaultOS já te conhece.
>
